...
uint16_t GPS_Conf600[2];                                             // GPS confidence 600cm in RAW tics to judge if better use spikefiltervalues
...
float    GPS_angle[2] = {0.0f, 0.0f};                                // it's the angles that must be applied for GPS correction
float    Last_GPS_angle[2] = {0.0f, 0.0f};
...
// Baro STATS LOGGING
        ...
        if (sensors(SENSOR_GPS) && sensors(SENSOR_MAG))                         // Only do GPS stuff if the Mag is available
        {
            ...
            {
                GPS_angle[ROLL]  = constrain((nav[LON] * cos_yaw_x - nav[LAT] * sin_yaw_y) * 0.1f,  -(float)maxbank10, (float)maxbank10);// Nav angles are in DEG * 100
                GPS_angle[PITCH] = constrain((nav[LON] * sin_yaw_y + nav[LAT] * cos_yaw_x) * 0.1f,  -(float)maxbank10, (float)maxbank10);// GPS_angle are expected to be in DEG * 10, I keep that for now
                for (axis = 0; axis < 2; axis++)
                {
                    if(!BlockGPSAngles && CopterFlying)
                    {
                        ...
                        GPS_angle[axis]      = (float)((int32_t)((tmp0flt * (1.0f - GPSEXPO) + tmp0flt * tmp0flt * tmp0flt * GPSEXPO) * (float)maxbank10)); // Do expo here, and some jitter cutoff
                    }
                    else
                    {
                        GPS_angle[axis]      = 0.0f;
                        Last_GPS_angle[axis] = 0.0f;
                    }
                }
            }
        }
        else GPS_angle[0] = GPS_angle[1] = 0.0f;
...
// But a little modified...
            ...
            axisPID[axis] = (float)((int32_t)(PTerm + ITerm - DTerm));          // Reduce Jitter
        }

...
static float bmp085_calculate(void)
{
    ...
    x1 = (((int32_t) bmp085_ut - (int32_t) bmp085.cal.ac6) * (int32_t) bmp085.cal.ac5) >> 15;
    x2 = ((int32_t) bmp085.cal.mc << 11) / (x1 + bmp085.cal.md);
    b5 = x1 + x2;
    BaroActualTemp = ((float)b5 + 8.0f) * 0.00625f; // Put out Temp (in C)
    b6 = b5 - 4000;
    // **** calculate B3 ***********
    x1 = (((b6 * b6) >> 12) * bmp085.cal.b2) >> 11;
    x2 = (bmp085.cal.ac2 * b6) >> 11;
    x3 = x1 + x2;
    b3 = (((((int32_t)bmp085.cal.ac1) * 4 + x3) << 3) + 2) >> 2; // the " * 4" will be optimized by compiler to bitshift
    // **** calculate B4 ***********
    x1 = (bmp085.cal.ac3 * b6) >> 13;
    x2 = (bmp085.cal.b1 * ((b6 * b6) >> 12) ) >> 16;
    x3 = ((x1 + x2) + 2) >> 2;
    b4 = (bmp085.cal.ac4 * (uint32_t) (x3 + 32768)) >> 15;
    b7 = ((uint32_t)bmp085_up - b3) * (50000 >> 3);
    if(!b4) return 0.0f;

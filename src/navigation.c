...
#define MagicEarthNumber       1.113195f                           // The "new" apm number
...
static float     Real_GPS_speed[2] = {0.0f, 0.0f}; // Is the earthframespeed measured by GPS Coord Difference
static float     MIX_speed[2]      = {0.0f, 0.0f}; // That is a 1:1 Mix of Acc speed and GPS Earthframespeed
static int32_t   Last_Real_GPS_coord[2];
volatile uint32_t TimestampNewGPSdata;    // Crashpilot in micros

// Earth / Location constants
static float     OneCmTo[2] = {0.0f, 0.0f};// Moves one cm in Gps coords
...
void GPS_calc_posholdCrashpilot(bool overspeed)
{
    ...
    for (axis = 0; axis < 2; axis++)
    {
        if (overspeed)
        {
            ...
            maxbank100new = (int16_t)((float)maxbankbrake100 * tmp0);
        } else maxbank100new = maxbank10 * 10;
        ...
        nav[axis]   = (float)constrain((int16_t)tilt, -maxbank100new, maxbank100new);
    }
}
...
void GPS_calc_nav_rate(uint16_t max_speed)
{
    ...
    float   temp, trgtspeed;
    uint8_t axis;
    int32_t crosstrack_error;
    if ((abs(wrap_18000(target_bearing - original_target_bearing)) < 4500) && cfg.nav_ctrkgain)// If we are too far off or too close we don't do track following
    {
        temp = (float)(target_bearing - original_target_bearing) * RADX100;     // temp is in RAD now
    ...
    for (axis = 0; axis < 2; axis++)
    {
        ...
        if (cfg.nav_tiltcomp)                                                   // Do the apm 2.9.1 tiltcompensation
        {
            temp = trgtspeed * trgtspeed * ((float)cfg.nav_tiltcomp * 0.0001f);
            if(trgtspeed < 0.0f) temp = -temp;
            nav[axis] += temp;
        }                                                                       // Constrain is done at the end of all GPS stuff
...
// ***   Utilities   ***
////////////////////////////////////////////////////////////////////////////////////
void GPS_set_pids(void)                                                         // Is done after EEPROM read Setup Constants
{
    ...
    GPS_Conf600[LAT]           = (uint16_t)(600.0f * OneCmTo[LAT]);
    GPS_Conf600[LON]           = 0;                                             // Will be calculated in "GPS_calc_longitude_scaling"
    maxbank10                  = (int16_t)cfg.gps_maxangle * 10;                // Initialize some values here
    GPSEXPO                    = (float)cfg.gps_expo * 0.01f;    
    if (cfg.gps_ph_brakemaxangle > cfg.gps_maxangle) maxbankbrake100 = maxbank10 * 10;
    else maxbankbrake100       = (int16_t)cfg.gps_ph_brakemaxangle * 100;
}
...
void GPS_reset_nav(void)                                                        // reset navigation (stop the navigation processor, and clear nav)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        nav[i]       = 0.0f;
        GPS_angle[i] = 0.0f;
        Last_GPS_angle[i] = 0.0f;
        Real_GPS_speed[i] = 0.0f;
...
void GPS_calc_longitude_scaling(bool force)
{
    ...
    if (!ini && Real_GPS_coord[LAT])                                            // We must have a GPS LAT coord to work!
    {
        ini  = true;
        ...
        OneCmTo[LON]     = 1.0f / (MagicEarthNumber * CosLatScaleLon);          // Moves EAST one cm Note: Can not be negative.
        GPS_Conf600[LON] = (uint16_t)(600.0f * OneCmTo[LON]);
    }
}
